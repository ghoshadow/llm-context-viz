...
test('builds stable category rows with labels and detail text', () => {
  ...
  assert.deepEqual(rows.map((row) => row.key), ['sysPrompt', 'tool_defs', 'mcp']);
  assert.deepEqual(rows.map((row) => row.label), ['系统提示', '工具定义', 'MCP / 插件']);
  assert.equal(rows[1]?.detailKey, 'codex.tools');
  assert.equal(rows[1]?.detail, '# tools');
  assert.equal(sumCalibrationCategoryChars(rows), 350);
});
